...
FILE_PATH = "/Users/selvi.arora/Desktop/Audio_Speech_Actors_01-24 2/Actor_01/03-01-03-02-02-01-01.wav"
SR = 16000  # sample rate

# Load and trim audio
wav, sr = librosa.load(FILE_PATH, sr=SR)
wav, _ = librosa.effects.trim(wav, top_db=20)  # remove quiet parts
wav = wav.astype(np.float32)

print(f"[load] sr={sr} Hz duration={len(wav)/sr:.2f}s shape={wav.shape}")

# Load processor and model with hidden states enabled
...
with torch.no_grad():
    out = model(**inputs)  # has .hidden_states
...
# Select layer 6 and pool
H = out.hidden_states[3]  # [1, T, 768]
# average mid layers 5–8
#mids = [out.hidden_states[i] for i in (5,6,7,8)]  # four layers
#H = torch.stack(mids, dim=0).mean(dim=0)
#embedding = H.mean(dim=1).squeeze(0)  # → [768]
X = H.squeeze(0)                        # [T, 768]
...
# Save
out_path = "../embeddings/happy/actor01_layer3_energy_trim.npy"
